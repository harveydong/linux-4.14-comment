...
uint32_t my_handler(void* data)
{
    ...
    p_my_info->cnt++;
    if(p_my_info->cnt%200==0) printf("cnt=%d (intr index %d)\n",p_my_info->cnt,p_my_info->index);

    if(p_my_info->cnt >= intr_test_cnt)
    {
        p_my_info->running = 0;
        printf("my handler %d exit, cnt=%d\n",p_my_info->index,p_my_info->cnt);
        return 0; //info usrdrv_lib irq_thread goto exit
    }
    return 1; //info usrdrv_lib irq_thread keep running
}
...
int main(int argc, char *argv[])
{
    ...
    usr_map_phys_addr(fd, (void**)&g_virt_io, (void**)&g_virt_mem_cached, (void**)&g_virt_mem_ncached);
    printf("g_virt_io=%p\n",g_virt_io);
    ...
    if(argc>1) intr_test_cnt = atoi(argv[1]);
    for(irq_index=0; irq_index < max; irq_index++)
    {
        ...
        usr_request_irq(my_info[irq_index].fd, irq_index, "usrdrv irq", &g_intr_info[irq_index], my_handler, &my_info[irq_index]);
    }
