							 ...
							 uint32_t output_freq)
{
	...
	if (!flag)
	{
		...
		pr_debug("D-PHY: Approximated Frequency: %d KHz\n",
							 ...
							 uint32_t output_data_rate)
{
	...
	gen3_if_write_word(dev, GEN3_IF_TESTER, RESET);
	gen3_if_write_word(dev, GEN3_IF_TESTER, GLUELOGIC);
	mipi_dsih_dphy_test_clear(dev, 1);
	mipi_dsih_dphy_test_clear(dev, 0);
	...
	{
		data[0] = 0x0F;
		mipi_dsih_dphy_write(dev, 0x30, data, 1);
	}

	//28;(Fcfg_clk(MHz)-17)*4]
	data[0] = ((phy->reference_freq / 1000) - 17) * 4;
	mipi_dsih_dphy_write(dev, 0x02, data, 1);

	if (phy->is_g118)
	{
		data[0] = (ranges[range].hs_freq);
		mipi_dsih_dphy_write(dev, 0x03, data, 1);
	}
	else
	{
		/* SOC PLL*/
		data[0] = 0x3F;
		mipi_dsih_dphy_write(dev, 0x20, data, 1);
	}

	//RESET RX
	gen3_if_write_word(dev, GEN3_IF_TESTER, RESET);
	gen3_if_write_word(dev, GEN3_IF_TESTER, RX_PHY);
	mipi_dsih_dphy_test_clear(dev, 1);
	mipi_dsih_dphy_test_clear(dev, 0);

	//RESET TX
	gen3_if_write_word(dev, GEN3_IF_TESTER, RESET);
	gen3_if_write_word(dev, GEN3_IF_TESTER, TX_PHY);
	mipi_dsih_dphy_test_clear(dev, 1);
	mipi_dsih_dphy_test_clear(dev, 0);
	...
	{
		/* DPHY GEN3 8 bits */
		pr_info("Configuring for Gen3 8-bits\n");
		...
		if (ranges[range].data_rate >= 1150)
			data[0] = 0x0E;
		else
			data[0] = 0x0D;
		...
		data[0] = (0x00 << 6) | (0x01 << 5) | (0x01 << 4);
		if (ranges[range].data_rate > 1250)
			data[0] = 0x00;
		mipi_dsih_dphy_write(dev, 0x19, data, 1);
	...
	else
	{
		...
		/* [7:6] reserverd | [5] hsfreqrange_ovr_en_rw |
		 * [4:1] target_state_rw | [0] force_state_rw */
		data[0] = (1 << 5);
		mipi_dsih_dphy_write(dev, 0x01, data, 1);

		/* PLL Lock Configurations */
		data[0] = 0x02;
		mipi_dsih_dphy_write(dev, 0x173, data, 1);
		data[0] = 0x00;
		mipi_dsih_dphy_write(dev, 0x174, data, 1);
		data[0] = 0x60;
		mipi_dsih_dphy_write(dev, 0x175, data, 1);
		data[0] = 0x03;
		mipi_dsih_dphy_write(dev, 0x176, data, 1);
		data[0] = 0x01;
		mipi_dsih_dphy_write(dev, 0x166, data, 1);
		...
		if (ranges[range].data_rate > 640)
			data[0] =
				((0 << 7) | (ranges[range].vco_range << 1) | (1 << 0));
		else
			data[0] =
				((1 << 7) | (ranges[range].vco_range << 1) | (1 << 0));
		mipi_dsih_dphy_write(dev, 0x17b, data, 1);

		data[0] = 0x10;
		mipi_dsih_dphy_write(dev, 0x15e, data, 1);
		data[0] = 0x04;
		mipi_dsih_dphy_write(dev, 0x162, data, 1);
		data[0] = 0x0C;
		mipi_dsih_dphy_write(dev, 0x16e, data, 1);

		/* Slew-Rate*/
		data[0] = 0x04;
		mipi_dsih_dphy_write(dev, 0x26b, data, 1);
		pr_debug("input divider %ld\n", input_divider);
		pr_debug("loop_divider %ld\n", loop_divider);
		/* pll_n_ovr_en_rw | PLL input divider ratio [6:3] |
		 * pll_tstplldig_rw*/
		data[0] =
			(1 << 7) | ((uint8_t)(input_divider - 1)) << 3;
		mipi_dsih_dphy_write(dev, 0x178, data, 1);

		/* PLL loop divider ratio [7:0] */
		data[0] = (uint8_t)(loop_divider - 2);
		mipi_dsih_dphy_write(dev, 0x179, data, 1);

		/* PLL loop divider ratio [9:8] */
		data[0] = (uint8_t)((loop_divider - 2) >> 8);
		mipi_dsih_dphy_write(dev, 0x17a, data, 1);

		if (ranges[range].data_rate < 450)
			data[0] = 0x1B;
		else
			data[0] = 0x0B;
		mipi_dsih_dphy_write(dev, 0x1AC, data, 1);
	}
	...
	if (ranges[range].data_rate > 648)
		mipi_dsih_dphy_delay(dev, 5);
	else
		mipi_dsih_dphy_delay(dev, 4);
						  ...
						  uint8_t data_length)
{
	...
	if (data != 0)
	{
		if (dev->phy.is_g118)
		{
			/* set the TESTCLK input high in preparation to
			 * latch in the desired test mode */
			mipi_dsih_dphy_test_clock(dev, 1);
			/* set the desired test code in the input 8-bit
			 * bus TESTDIN[7:0] */
			mipi_dsih_dphy_test_data_in(dev,
										(uint8_t)address);
			/* set TESTEN input high  */
			mipi_dsih_dphy_test_en(dev, 1);
			/* drive the TESTCLK input low; the falling edge
			 * captures the chosen test code into the
			 * transceiver */
			mipi_dsih_dphy_test_clock(dev, 0);
			/* set TESTEN input low to disable further test
			 * mode code latching  */
			mipi_dsih_dphy_test_en(dev, 0);
			/* start writing MSB first */
			for (i = data_length; i > 0; i--)
			{
				/* set TESTDIN[7:0] to the desired test
				 * data appropriate to the chosen test
				 * mode */
				mipi_dsih_dphy_test_data_in(dev,
											data[i - 1]);
				/* pulse TESTCLK high to capture this
				 * test data into the macrocell; repeat
				 * these two steps as necessary */
				mipi_dsih_dphy_test_clock(dev, 1);
				mipi_dsih_dphy_test_clock(dev, 0);
			}
		}
		else
		{
			...
			mipi_dsih_dphy_test_en(dev, 1);
			mipi_dsih_dphy_test_clock(dev, 1);
			mipi_dsih_dphy_test_data_in(dev, 0x00);
			mipi_dsih_dphy_test_clock(dev, 0);
			mipi_dsih_dphy_write_word(dev,
									  R_DPHY_TST_CRTL1, 0);
			mipi_dsih_dphy_write_word(dev,
									  R_DPHY_TST_CRTL1, (uint8_t)(address >> 8));
			mipi_dsih_dphy_test_clock(dev, 1);
			mipi_dsih_dphy_test_clock(dev, 0);
			mipi_dsih_dphy_test_en(dev, 1);
			mipi_dsih_dphy_test_clock(dev, 1);
			...
			for (i = data_length; i > 0; i--)
			{
				mipi_dsih_dphy_test_data_in(dev, ((uint8_t)data[i - 1]));
				mipi_dsih_dphy_test_clock(dev, 1);
				mipi_dsih_dphy_test_clock(dev, 0);
			}
		}
...
void mipi_dsih_dphy_delay(struct mipi_dsi_dev *dev, int value)
{
	...
	if (!phy->is_g118)
	{
		...
		mipi_dsih_dphy_write(dev, R_12BITS_DPHY_RDY_L0, data, 1);
		mipi_dsih_dphy_write(dev, R_12BITS_DPHY_RDY_L1, data, 1);
		mipi_dsih_dphy_write(dev, R_12BITS_DPHY_RDY_L2, data, 1);
		mipi_dsih_dphy_write(dev, R_12BITS_DPHY_RDY_L3, data, 1);
	...
	else
	{
		...
		mipi_dsih_dphy_write(dev, R_8BITS_DPHY_RDY_L0, data, 1);
		mipi_dsih_dphy_write(dev, R_8BITS_DPHY_RDY_L1, data, 1);
		mipi_dsih_dphy_write(dev, R_8BITS_DPHY_RDY_L2, data, 1);
		mipi_dsih_dphy_write(dev, R_8BITS_DPHY_RDY_L3, data, 1);

		pr_debug("%d clk cyles of delay on READY signal - 8bits. \n",
				 value);
	}
}
void mipi_dsih_dphy_mmcm_pclk(struct mipi_dsi_dev *dev, uint32_t pclk)
{
	...
	rst = mmcm_read_word(dev, DRP_POWER_REG);
	mmcm_write_word(dev, DRP_POWER_REG, 0xFFFF);

	switch (pclk)
	{
	case 5000:
	{ // (100 / 20) * 1
		data = mmcm_read_word(dev, DRP_CLKOUT0_REG_1);
		temp_0 = data & MMCM_MASK;
		temp_1 = (temp_0 | (0x14 << 6) | (0x14)); // 20 divider
		mmcm_write_word(dev, DRP_CLKOUT0_REG_1, temp_1);
		mmcm_read_word(dev, DRP_CLKOUT0_REG_2);

		data = mmcm_read_word(dev, DRP_CLKFBOUT_REG_1);
		temp_0 = data & MMCM_MASK;
		temp_1 = temp_0 | (0x1 << 6) | (0x1); // 1 Multiplier
		mmcm_write_word(dev, DRP_CLKFBOUT_REG_1, temp_1);
		mmcm_read_word(dev, DRP_CLKFBOUT_REG_1);
		mmcm_read_word(dev, DRP_CLKFBOUT_REG_2);
		break;
	}
	case 10000:
	{ // (100 / 50) * 5
		data = mmcm_read_word(dev, DRP_CLKOUT0_REG_1);
		temp_0 = data & MMCM_MASK;
		temp_1 = (temp_0 | (0x32 << 6) | (0x32)); // 50 divider
		mmcm_write_word(dev, DRP_CLKOUT0_REG_1, temp_1);
		mmcm_read_word(dev, DRP_CLKOUT0_REG_2);

		data = mmcm_read_word(dev, DRP_CLKFBOUT_REG_1);
		temp_0 = data & MMCM_MASK;
		temp_1 = temp_0 | (0x5 << 6) | (0x5); // 5 Multiplier
		mmcm_write_word(dev, DRP_CLKFBOUT_REG_1, temp_1);
		mmcm_read_word(dev, DRP_CLKFBOUT_REG_1);
		mmcm_read_word(dev, DRP_CLKFBOUT_REG_2);
		break;
	...
	{ // (100 / 40) * 10
		data = mmcm_read_word(dev, DRP_CLKOUT0_REG_1);
		temp_0 = data & MMCM_MASK;
		temp_1 = (temp_0 | (0x28 << 6) | (0x28)); // 40 divider
		mmcm_write_word(dev, DRP_CLKOUT0_REG_1, temp_1);
		mmcm_read_word(dev, DRP_CLKOUT0_REG_2);

		data = mmcm_read_word(dev, DRP_CLKFBOUT_REG_1);
		temp_0 = data & MMCM_MASK;
		temp_1 = temp_0 | (0xA << 6) | (0xA); // 10 Multiplier
		mmcm_write_word(dev, DRP_CLKFBOUT_REG_1, temp_1);
		mmcm_read_word(dev, DRP_CLKFBOUT_REG_1);
		mmcm_read_word(dev, DRP_CLKFBOUT_REG_2);
		break;
	...
	case 85000:
	{
		data = mmcm_read_word(dev, DRP_CLKOUT0_REG_1);
		temp_0 = data & MMCM_MASK;
		temp_1 = (temp_0 | (0xC << 6) | (0xC)); // 40 divider
		mmcm_write_word(dev, DRP_CLKOUT0_REG_1, temp_1);
		mmcm_read_word(dev, DRP_CLKOUT0_REG_2);
	...
	default:
		pclk = 100;
		pr_debug("Invalid PCLK configurations. Default kept.\n");
		break;
	}
	pr_debug("PCLK configured for %d KHz.\n", pclk);
	mmcm_write_word(dev, DRP_POWER_REG, rst);
}
/* abstracting BSP */
/**
