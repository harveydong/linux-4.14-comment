											   ...
											   register_config_t *config, uint16_t config_length)
{
	...
	{
		/* dump all registers */
		for (current_val = R_DSI_HOST_VERSION;
			 current_val <= R_DSI_HOST_PHY_TMR_LPCLK_CFG;
			 count++, current_val += (R_DSI_HOST_PWR_UP - R_DSI_HOST_VERSION))
		{

			if ((config_length == 0) || (config == 0) || count >= config_length)
			{
				/* no place to write - write to STD IO */
				pr_info("DSI 0x%X:0x%X", current_val, mipi_dsih_read_word(dev, current_val));
			}
			else
			{
				config[count].addr = current_val;
				config[count].data = mipi_dsih_read_word(dev, current_val);
			}
		}
	}
	else
